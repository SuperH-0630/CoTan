...
class SheetFunc:
    def __init__(self, func, name, style):
        ...
        for i in range(len(func[0])):  # 检查
            try:
                ...
                pass
        iter_x = sorted(list(set(float_x_list)))  # 排序并且去除重复
        # 筛查重复
        x = []
        y = []
                ...
                try:
                    ...
                    if last_y is not None and last_monotonic != now_monotonic:
                        ...
                        if group_score >= 5 and (now_monotonic != 2 or balance != 2):
                            ...
                            self.__classification_x.append([])
                            self.__classification_y.append([])
                    last_monotonic = now_monotonic
                    self.__classification_x[-1].append(now_x)
                    self.__classification_y[-1].append(y)
                    last_y = y
                except BaseException:
                    pass
            ...
            if len(self.__classification_x[i]) == 1 and not must_forward:  # 检测到有单个群组
                ...
                if front_reason < back_reason:  # 前原因小于后原因，连接到前面
                    try:
                        ...
                        new_classification_y[-1] += self.__classification_y[i]
                    except BaseException:  # 按道理不应该出现这个情况
                        new_classification_x.append(self.__classification_x[i])
                        new_classification_y.append(self.__classification_y[i])
                else:
                    new_classification_x.append(self.__classification_x[i])
                    new_classification_y.append(self.__classification_y[i])
                    must_forward = True
    ...
    def __parity(self, *args, **kwargs):  # 奇偶性
        if not self.have_data_packet:
            self.data_packet()  # 检查Cul的计算
        x = self.__x.copy()
        left_x = sorted(x)[0]
    ...
    def __monotonic(self):  # 单调性
        if not self.have_data_packet:
            self.data_packet()  # 运行Cul计算
        classification_x = self.__classification_x.copy()
    ...
    def property_prediction(self, output_prompt=lambda x: x):
        answer = []
...
class ExpFunc:
    def __init__(
        ...
        # 函数求导
        try:
            self.derivatives = sympy.diff(self.func, self.x)
        except BaseException:
    ...
    def __best_value(self):  # 计算最值和极值点
        ...
        if len(self.__classification_x) != 1:  # 没有计算的必要
            ...
            if not self.best_r:
                pass
                return self.max_x, self.max_y, self.min_x, self.min_y
        y = self.__y_real + self.memore_y  # x和y数据对齐(因为是加法，所以y的修改不影响self.__ya)
    ...
    def __parity(self, precision=False):  # 启动round处理
        if not self.have_data_packet:
            self.data_packet()  # 运行Cul计算
        if len(self.__classification_x) != 1:
    ...
    def sympy_calculation(self, y_value):  # 利用Sympy解方程
        try:
            ...
            result_list = sympy.solve(equation, self.x)
            answer = []

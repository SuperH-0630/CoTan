...
class Page_Parser:
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_id(id)]  # 返回必须是list
            else:
                self.element_dict[f'{name}[{num}]'] = browser.find_elements_by_id(
                    id)
    ...
    def find_class(self, class_name, not_all=False, **kwargs):
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_class_name(class_name)]  # 返回必须是list
            else:
                self.element_dict[f'{name}[{num}]'] = browser.find_elements_by_class_name(
                    class_name)  # 返回必须是list
    ...
    def find_name(self, name_, not_all=False, **kwargs):
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_name(name_)]  # 返回必须是list
            else:
    ...
    def find_xpath(self, xpath, not_all=False, **kwargs):
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_xpath(xpath)]  # 返回必须是list
            else:
    ...
    def find_css(self, css_selector, not_all=False, **kwargs):
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_css_selector(css_selector)]  # 返回必须是list
            else:
                self.element_dict[f'{name}[{num}]'] = browser.find_elements_by_css_selector(
                    css_selector)  # 返回必须是list
    ...
    def find_tag_name(self, tag_name, not_all=False, **kwargs):
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_tag_name(tag_name)]  # 返回必须是list
            else:
    ...
    def find_link_text(self, link_text, not_all=False, **kwargs):  # 匹配link
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_link_text(link_text)]  # 返回必须是list
            else:
                self.element_dict[f'{name}[{num}]'] = browser.find_elements_by_link_text(
                    link_text)  # 返回必须是list
    ...
    def find_partial_link_text(self, partial_link_text, not_all=False, **kwargs):  # 模糊匹配
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if not_all:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_partial_link_text(partial_link_text)]  # 返回必须是list
            else:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.find_element_by_partial_link_text(partial_link_text)]  # 返回必须是list
        self.add_func(
            f'find_partial_link_text:{partial_link_text}',
            find)  # 添加func
    ...
    def find_switch_to_frame(self, reference, is_id=False, *args, **kwargs):  # 定位Frame
        ...
        def find(browser, num, name, *args, **kwargs):
            ...
            if reference is None:
                self.element_dict[f'{name}[{num}]'] = [
                    browser.default_content()]  # 回到主文档
            elif reference == '':
                self.element_dict[f'{name}[{num}]'] = [
                    browser.parent_frame()]  # 回到父文档
            else:
                if is_id:
                    reference = int(reference)
                self.element_dict[f'{name}[{num}]'] = [
                    browser.switch_to.frame(str(reference))]  # 定位进入文档
    ...
    def deselect_by_index(self, element_value, deselect, index=0, **kwargs):  # 根据index取消选择
        ...
        def action(*args, **kwargs):
            nonlocal self
            self.element_dict[element_value][index].deselect_by_index(
                int(deselect))
        self.add_func(
            f'deselect_by_index:{deselect}>{element_value}[{index}]',
            action)  # 添加func
    ...
    def deselect_by_text(self, element_value, deselect, index=0, **kwargs):  # 根据text取消选择
        ...
        def action(*args, **kwargs):
            nonlocal self
            self.element_dict[element_value][index].deselect_by_visible_text(
                deselect)
        self.add_func(
            f'deselect_by_text:{deselect}>{element_value}[{index}]',
            action)  # 添加func
    ...
    def deselect_by_value(self, element_value, deselect, index=0, **kwargs):  # 根据value取消选择
        ...
        self.add_func(
            f'deselect_by_value:{deselect}>{element_value}[{index}]',
            action)  # 添加func
    ...
    def select_by_index(self, element_value, deselect, index=0, **kwargs):  # 根据index选择
        ...
        def action(*args, **kwargs):
            nonlocal self
            self.element_dict[element_value][index].select_by_index(
                int(deselect))
        self.add_func(
            f'select_by_index:{deselect}>{element_value}[{index}]',
            action)  # 添加func
    ...
    def select_by_text(self, element_value, deselect, index=0, **kwargs):  # 根据text选择
        ...
        def action(*args, **kwargs):
            nonlocal self
            self.element_dict[element_value][index].select_by_visible_text(
                deselect)
        self.add_func(
            f'select_by_text:{deselect}>{element_value}[{index}]',
            action)  # 添加func
    ...
    def select_by_value(self, element_value, deselect, index=0, **kwargs):  # 根据value选择
        ...
        self.add_func(
            f'select_by_value:{deselect}>{element_value}[{index}]',
            action)  # 添加func
    ...
    def to_text(self, **kwargs):  # 获取网页源码
        ...
        def action(num, name, *args, **kwargs):
            ...
            try:
                self.element_dict[f'{name}[{num}]'] = [
                    self.browser.page_source, self.now_url]
            except BaseException:
                self.element_dict[f'{name}[{num}]'] = [
                    self.browser.text, self.now_url]  # request
    ...
    def make_bs(self, element_value, **kwargs):  # 解析成bs4对象
        ...
        def action(num, name, *args, **kwargs):
            nonlocal self
            self.element_dict[f'{name}[{num}]'] = [bs4.BeautifulSoup(
                self.element_dict[element_value][0], "html.parser")]
    ...
    def findAll(self, element_value, tag: (str, list), attribute: dict, limit, recursive, index: (slice, int), **kwargs):  # 根据标签定位
        ...
        def action(num, name, *args, **kwargs):
            ...
            for bs in iter_list:
                try:
                    re = bs.find_all(
                        tag, attribute, limit=limit, recursive=recursive)
                except BaseException:
                    try:
                        ...
                        for agrs_name in attribute:
                            ...
                            if isinstance(text, str):
                                ...
                                if not regular.match(
                                        text, bs.attrs[agrs_name]):
                                    raise Exception
                        re = [bs]
                    except BaseException:
                        re = []
    ...
    def findAll_by_text(self, element_value, text: (regular.compile, str), limit, recursive, index: (slice, int), **kwargs):  # 根据text定位
        ...
        def action(num, name, *args, **kwargs):
            ...
            for bs in iter_list:
                try:
                    re = bs.find_all(
                        text=text, limit=limit, recursive=recursive)
                except BaseException:
                    try:
                        if isinstance(text, str):
            ...
            self.element_dict[f'{name}[{num}]'] = paser_list
        self.add_func(
            f'findAll_by_text:{element_value}[{index}]',
            action)  # 添加func
    ...
    def __get_other_base(self, element_value, index: (slice, int), who='children', **kwargs):  # 获得子、后代、兄弟标签的基类
        ...
        def action(num, name, *args, **kwargs):
            ...
            for bs in iter_list:
                if who != 'brothers':
                    paser_list += {'children': bs.children,
                                   'offspring': bs.descendants,
                                   'down': bs.next_siblings,
                                   'up': bs.previous_siblings}.get(who,
                                                                   bs.children)
                else:
    ...
    def add_url(self, element_value, index: (slice, int), url_name, update_func, url_args: dict, **kwargs):  # 自动添加url
        ...
        self.add_func(
            f'add_URL<{element_value}[{index}]:{url_name}',
            action)  # 添加func
    ...
    def ActionChains_click(self, Chains, element_value, index, **kwargs):  # 单击左
        ...
        def action(*args, **kwargs):
            nonlocal self
            self.element_dict[Chains][0].click(
    ...
    def ActionChains_key_up(self, Chains, key, element_value, index, is_special_keys, **kwargs):  # down
        ...
        def action(*args, **kwargs):
            ...
            self.element_dict[Chains][0].key_up(
                new_key, self.element_dict[element_value][index])
        self.add_func(
            f'[{Chains}]key_up>{key}:[{element_value}][{index}]',
            action)  # 添加func
